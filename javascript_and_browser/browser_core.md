...
| スレッド名            | 概要                                                                 |
|----------------------|----------------------------------------------------------------------|
| メインスレッド (Main Thread) | **JavaScript の実行**や **DOM の操作**、**レンダリング** を担当。                     |
| レンダリングスレッド (Rendering Thread) | **CSS の適用**や **描画処理** を担当。                                 |
| ワーカースレッド (Worker Thread) | **Web Worker** や **Service Worker** など、JavaScript の非同期処理を分担。          |
| I/O スレッド          | **ネットワーク通信**（`fetch` や `XHR`）や **ファイル読み書き** を担当。                    |
| タイマースレッド      | **タイマー処理**（`setTimeout` や `setInterval`）のカウントを管理。                       |
| ガベージコレクタスレッド (Garbage Collector) | **メモリ管理**や不要になったオブジェクトの破棄をバックグラウンドで実行。         |
